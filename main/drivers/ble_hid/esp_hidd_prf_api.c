...
// HID径向旋钮 报告长度：1字节 + 2字节(拨盘值)
#define HID_RADIAL_IN_RPT_LEN     3
...
void esp_hidd_send_keyboard_value(uint16_t conn_id, key_mask_t special_key_mask, uint8_t *keyboard_cmd, uint8_t num_key)
{
    ...
    hid_dev_send_report(hidd_le_env.gatt_if, conn_id,
                        HID_ITF_PROTOCOL_KEYBOARD, HID_REPORT_TYPE_INPUT, HID_KEYBOARD_IN_RPT_LEN, buffer);
    return;
}
...
void esp_hidd_send_mouse_value(uint16_t conn_id, uint8_t mouse_button, int8_t mickeys_x, int8_t mickeys_y)
{
    ...
    hid_dev_send_report(hidd_le_env.gatt_if, conn_id,
                        HID_ITF_PROTOCOL_MOUSE, HID_REPORT_TYPE_INPUT, HID_MOUSE_IN_RPT_LEN, buffer);
    return;
}
...
/**
 *
 * @param hid_conn_id 连接id
 * @param buttons 是否按下
 * @param rotation 旋转值
 */
void ble_hid_surfacedial_report_custom(uint16_t hid_conn_id, uint8_t buttons, int16_t rotation)
{
    const uint8_t coe = 3;
    uint8_t report[2];
    int16_t dial_step;
...
void ble_hid_surfacedial_report(uint8_t hid_conn_id, uint8_t keycode)
{
    uint8_t report[2];
    int16_t dial_step;
    if(keycode<2)
